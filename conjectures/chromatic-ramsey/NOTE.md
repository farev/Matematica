...
the first exact values, F(3,3) = 14, F(3,4) = 41 and F(4,3) = 16, and
certify F(3,5) ≥ 122, F(3,6) ≥ 365, F(4,4) ≥ 44. Wiesner's formula is exact
at (3,3) and (3,4), a strict lower bound at (4,3), and its j = 3 value
(3^k+1)/2 is realised for k ≤ 6 by the set of ternary words with an even
number of 2's. Whether that set works for every k, which would give
...
| F(3,4) | **41** | `witness_F3_4_n41.txt`, `col_even0_k4.txt`, circulant `circ_41_4_3.json` | ≤ 42 by Lemma 2.1(a) from F(3,3) = 14 (PROVED); ≤ 41 by the cube-and-conquer refutation of Theorem 5.2 |
| F(3,5) | ≥ 122 | `col_even0_k5.txt`, circulant `circ_122_5_3.json` | ≤ 3·41 = 123 |
| F(3,6) | ≥ 365 | `col_even0_k6.txt` (365 vertices, 8,038,030 triples checked) | ≤ 3·F(3,5) ≤ 369 |
...
**Theorem 5.2 (F(3,4) ≤ 41; CERTIFIED).** No 42-point subset of [3]^4 admits
a valid colouring.

*Proof.* Let S be a 42-point valid configuration. Each of the three layers
S ∩ {x_4 = v} is a valid configuration of [3]^3 (pairs inside a layer agree
in coordinate 4, so their colours lie in {1,2,3}), hence has at most
F(3,3) = 14 points; so every layer has exactly 14. The layer x_4 = 0 is
therefore one of the colourable 14-sets of [3]^3, whose complete list —
33,831 sets forming 37 orbits under S_3 ≀ S_3 (value permutations in each of
the first three coordinates and permutations of those coordinates) — is
certified: `enum_certified.py` solves the "exactly 14 selected" model
incrementally with blocking clauses, and the final UNSAT (base model plus all
33,831 blocking clauses, 37,097 clauses) has a 1,529,304-line DRUP proof
checked by `rup_check` (3 min 59 s), so no colourable 14-set is missing. The
symmetry group extends to [3]^4 by fixing coordinate 4, so after applying an
element of it we may assume the layer x_4 = 0 equals one of the 37 orbit
representatives R. For each R, the model "select exactly the types (r,0),
r ∈ R, in layer 0; at least 42 types in all; valid colouring; the implied
bounds of Lemma 2.1 (≤ 2 per line, ≤ 5 per plane, ≤ 14 per 3-cube)" is
unsatisfiable: CaDiCaL refutes the 37 cubes in 228 s in total, and Glucose 4
refutes each again with a DRUP proof (167 s in total; proofs of 0.3–37 MB,
380 MB altogether), every one checked by `rup_check`
(`certs/cube42_verification.log`, hashes in `certs/SHA256SUMS_large.txt`). The base CNF and the
27 unit clauses of each cube are in `certs/` (`F34_n42_base.cnf`,
`F34_n42_cube_units.json`); the proofs are regenerated by
`cube42.py reps_3_3_14.json --prove`. ∎

A single plain SAT run of the n = 42 instance (with the same implied bounds)
did not finish in 50 minutes of CaDiCaL nor 73 minutes of proof-logged
Glucose; the split by the layer type is what makes the refutation cheap.
...
2 ≤ k ≤ 6, and O_k for 2 ≤ k ≤ 4 (files `col_even0_k*.txt`, `col_even1_k*.txt`,
all verified from the definition; E_6 has 365 vertices, 10.9 M clauses,
238 s of CaDiCaL). Hence F(3,k) ≥ (3^k+1)/2 for k ≤ 6, with equality for
k ≤ 4. For comparison, Theorem 3.1 gives 12, 32, 80, 240 at k = 3, 4, 5, 6
against 14, 41, 122, 365.
...
for k ≤ 4; whether the ratio F(3,k)/3^k tends to 1/2 or below is open. Note
F(3,k)/3^k is non-increasing (Corollary 2.2) with F(3,4)/81 = 41/81 ≈ 0.506,
so lim F(3,k)/3^k ≤ 41/81; if F(3,5) = 122 the bound drops to 122/243 ≈ 0.502.
...
2. Is F(3,k) = (3^k+1)/2 for all k? The first undecided cell is F(3,5) ∈
   {122, 123}; no circulant witness for 123 exists (SAT, 0.1 s), and the
   cube-and-conquer of Theorem 5.2 would need the census of extremal
   41-sets of [3]^4 first.
