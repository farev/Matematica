...
| **disc(T) ≤ 7 = U(29) for every triangulation on 29 vertices** (the fourth open order; ≈ 4·10¹² triangulations): by Theorem 3 a counterexample has exactly three high-degree big-class vertices, so it is found among plantri's 977,526,957 triangulations on 18 vertices with three non-adjacent vertices of degree 5–6; 60,070,002 fully-mixed configurations, all of discrepancy 1, all admitting a single flip (the last case not covered by the proof; ~7 core-hours) | CERTIFIED | NOTE Theorem 6; `hstruct.c`, `results_struct_m4.txt` |
