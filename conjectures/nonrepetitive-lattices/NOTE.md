...
Certificate: `certs/tri_hex3_c7.cnf` (925 variables, 265 459 clauses, 259 350 path clauses of which 241 053 lazily generated over 193 iterations, 210 s), encoding validated; Glucose 4.2 UNSAT in 198 s with a DRUP proof of 2 605 263 lines (170 MB), `s VERIFIED` by drat-trim (234 s; 20 069 of the 265 459 clauses and 1 136 690 of 1 330 307 lemmas in the core, no RAT lemmas) and by rup_check (762 s). The full proof is not committed (25 MB even trimmed and compressed, above this repository's size limit); it regenerates from the committed CNF with `certify.py` in about four minutes. The unsatisfiable core with all structural clauses restored, `certs/tri_hex3_c7_core.cnf` (22 121 clauses: 16 149 path clauses on 493 edges, encoding validated), is also committed; Glucose refutes it in 77 s with a proof of 2 547 954 lines, `s VERIFIED` by drat-trim (172 s) and by rup_check (383 s), so the shorter instance does not give a shorter proof and its proof is likewise not committed. The radius-2 hexagon `H₂` (19 vertices, 42 edges) already has no nonrepetitive 6-colouring: `certs/tri_hex2_c6.cnf` (271 variables, 8 489 clauses), proof 4 946 lines, both checkers `s VERIFIED`; this gives `π(T₃) ≥ 7` with a 0.1 s solve.
...
- King's graph, 9 colours: the 30-vertex spiral `K₃₀` has a 9-colouring with no repetition on paths of at most 16 vertices (`king_sp30_c9.sat.txt`); the 6×6 block with paths of at most 10 vertices likewise. The 7×7 block (all paths of at most 6 vertices eager, repetitions up to 14 vertices lazy) was stopped without a verdict after 42 CPU-minutes; the 8×8 block's first solve did not finish in 6 minutes.
- Triangular lattice, 8 colours: `H₃` has an 8-colouring with no repetition on paths of at most 16 vertices (`tri_hex3_c8.sat.txt`, 1 901 iterations, 490 000 lazy clauses, 33 s). On `H₄` (61 vertices, 150 edges) with repetitions up to 20 vertices the lazy loop was stopped after 1 161 iterations and 5.75 million lazy clauses (514 s, every iteration still producing 5 000 new repetitive paths); no verdict.
