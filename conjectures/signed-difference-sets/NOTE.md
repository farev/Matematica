...
wall time; certificates in `certs/`). Summary at session close is in
README.md. Every EXIST row ships an explicit witness verified by the
independent checker; every NONEXIST row is an exhaust with node counts,
plus — where the criteria also apply — an independent proof.
...
is an export-stage corruption of the witness lists. The 21 affected
...
4. Existence side: He–Chen–Ge's PDS constructions populate ten cells;
   none of today's exhausted small cells contained a sporadic SDS. Is
   there a nonexistence heuristic (e.g. λ large relative to k−λ) that
   the decided shelf supports?
