...
| Claim | Label | Where |
|---|---|---|
| Undirected exponents `≥ (k−1)/(k−2)` are unavoidable over `Σ_k` beyond length `k+3`, for `k = 22, 23, 24, 25` (exhaustive 451/483/516/550-node trees) — hence `URT(k) ≥ (k−1)/(k−2)` at these `k`, independent of the literature | **CERTIFIED** | NOTE §4, `lower_bounds.py` |
...
Max U-`α`-free length `= k+3` for all `k ≥ 22` is **Conjecture C3** (data:
`k ≤ 25`).
