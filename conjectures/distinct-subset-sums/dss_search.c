/* dss_search.c — exhaustive search for n-element sets of positive integers
 ...
 * Words below a>>6 receive sh = 0, so they can never collide and are
 * copied wholesale; the test loop starts at the shift word. */
static inline int add_elem(const u64 *S, int nw, int a, u64 total,
                           u64 *out, int *out_nw)
{
    int q = a >> 6, s = a & 63;
    int nw2 = (int)((total + (u64)a) >> 6) + 1;
    for (int i = 0; i < q && i < nw; i++) out[i] = S[i];
    for (int i = (q < nw ? q : nw); i < q && i < nw2; i++) out[i] = 0;
    for (int i = q; i < nw2; i++) {
        u64 lo = (i - q < nw) ? S[i - q] : 0;
...
static void search(State *st, Stats *stats)
{
    ...
    if (hi < lo) return;
    /* P2/P3 terms, updated incrementally: g(a) = sum_{j=0..r-1}(a-j),
     * h(a) = sum_{j=0..r-1}(a-j)^2; g(a-1) = g-r, h(a-1) = h-2g+r */
    u64 g = 0, h = 0;
    for (int j = 0; j <= r - 1; j++) {
        u64 v = (u64)(hi - j);
        g += v; h += v * v;
