...
| Claim | Label | Where |
|---|---|---|
| Complete census of A030979 below 3^600 ≈ 1.87·10^286: ⟨PENDING-600⟩ terms (published frontier: 1374 terms below 10^70) | CERTIFIED | NOTE §3 C1, `data/` |
...
| Count table G(3^k), k ≤ 600 — first counts beyond 10^70 — with local-exponent bands vs the 0.02595 heuristic | CERTIFIED (counts) / NUMERICAL (fit) | NOTE §3 C3, C5, `data/counts600.txt` |
...
| Largest known term of A030979: ⟨PENDING-600: digits⟩-digit number | CERTIFIED | `data/max600.txt` |
...
| `engine_td.c` | production top-down engine: digit-array state, O(1) prune, full/taskgen/tasks/max/tables modes | 18 s at 3^300; ⟨PENDING-600⟩ at 3^600 | the census ladder (`data/prod*`) |
