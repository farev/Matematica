...
| [**Additive squares (PVHH)**](conjectures/additive-squares/) | active | 1 | **PROVED** a quotient lemma making additive-square-freeness depend only on the alphabet's relation lattice — it re-derives Freedman's bound `L ≤ 60` for `a+d=b+c` from one relation vector and gives `L = 7` for every 3-letter alphabet — plus **CERTIFIED** exact maxima for 34 four-letter integer alphabets (first such table) with 60 shown attained at `{0,1,5,6}` |
