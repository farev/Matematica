...
| [**Plus-minus Davenport constants**](conjectures/plus-minus-davenport/) | active | 1 | **CERTIFIED** — the last unknown plus-minus weighted Davenport constant of order ≤ 100 decided: **D±(C5⊕C15) = 6** (the one cell Marchan–Ordaz–Schmid 2014 left open, "either 6 or 7"), by six independent exhaustive computations across three methods, with lower bound **PROVED**; plus **D±(C7⊕C21) = 8** (first open case of the next family; upper bound PROVED via the subset-sums cap), a certified table of dissociation numbers for 300+ groups of rank ≤ 4 whose rank-2 values all sit at an endpoint of the proved floor/cap window (only C3⊕C3 and C5⊕C15 stuck at floor), and a **PROVED** reduction (Lemma R) localizing the 75-vs-147 contrast to a subset-sum statement over F_p² |
