...
| [**Chromatic number of PG(7,2)**](conjectures/projective-chromatic/) | active | 1 | **PROVED + CERTIFIED** — first computational attack on χ₂(8) ∈ {5,6} (Bishnoi–Cames van Batenburg–Ravi Problem 1, would give R(3;5) ≥ 257 if 5): a Mersenne-order obstruction proved (no 5-coloring of PG(7,2) is invariant under any collineation of order 3, 7, 31 or 127 — any n, k), order-17 and Frobenius invariance excluded with DRUP-verified certificates, an order-5-invariant witness shown to **exist at n = 7** (the contrast level), and 1,000/1,000 sampled hyperplane restrictions non-extendable — with the full "2-group stabilizer" theorem pending two order-5 UNSAT runs |
