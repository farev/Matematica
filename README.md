...
| [**Plus–minus Davenport constants**](conjectures/plus-minus-davenport/) | active | 1 | **CERTIFIED** — the two smallest reported-open plus–minus weighted Davenport constants decided: **`D±(C₅⊕C₁₅) = 6`** (reportedly the last unknown group of order ≤ 100 (secondary); the counting bound is *not* attained — three independent exhaustions) and **`D±(C₇⊕C₂₁) = 8`** (attained, and the extremal 7-set is *unique up to automorphism*: 2016 maximum sets = one Aut-orbit); plus the first census of max dissociated-set size for all **493** abelian groups of order ≤ 255 (484 attain `⌊log₂∣G∣⌋+1`; all 9 exceptions catalogued, incl. two where *neither* classical bound is tight) and **PROVED** closure of the family `D±(C_p⊕C_{3p})` for `p ≤ 17`, with `p = 5` the unique exception |
