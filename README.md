...
| [**Undirected repetition thresholds**](conjectures/undirected-thresholds/) | active | 1 | **CERTIFIED** — two-sided structure at the open frontier `k = 22` of the Currie–Mol conjecture `URT(k) = (k−1)/(k−2)`: exponents `≥ 21/20` are unavoidable over 22 letters beyond length exactly **25** (451-node exhaustive certificate, re-deriving the lower bound in-repo, likewise `k = 23, 24, 25` at length `k+3`), while a quadruply-verified `(21/20)⁺`-free word of length **20 000** exists; the binary Pansiot class and the whole affine-morphism ansatz are certified **empty**; plus **PROVED** — exact Pansiot reversal-transfer identities and a finite-check descent criterion (Theorem D) for any future uniform-morphic witness |
