...
| [**Consecutive 8th-power residues (Erdős #436)**](conjectures/power-residue-pairs/) | active | 1 | **CERTIFIED** — the first explicit upper bound **`Λ(8,2) ≤ [[U]]`** (Brillhart–Lehmer–Lehmer 1964 had only `≥ 1,200,744`): an exhaustive unit-orbit case tree over the 8th-power characters of small primes with `R(2)` even, [[leaves]] leaves all settled, independently checked; controls reproduce `Λ(2..7,2)` exactly and are sharp one below each |
