...
| [**Plus-minus Davenport constants**](conjectures/plus-minus-davenport/) | active | 1 | **CERTIFIED** — the last open plus-minus weighted Davenport constant at order ≤ 100 decided: **`D±(C₅⊕C₁₅) = 6`** (boxed {6,7} since Marchan–Ordaz–Schmid 2014, (secondary)), plus four more smallest-open family cases — `D±(C₇⊕C₂₁) = 8`, `D±(C₃⊕C₄₅) = 7`, `D±(C₅⊕C₅₅) = 8`, `D±(C₃⊕C₈₇) = 8` — each with every witness re-verified from the definition and upper bounds by exhaustion (up to 3.49 billion nodes); a from-scratch census of all **184** abelian groups of order ≤ 100 extended to 127 (exactly five deficits); **PROVED** attainment theorem T1 for `C₃⊕C₃ₙ` (presumptive MOS-regime rediscovery, marked) which with machine decisions settles that family for **every n ≤ 56**, moving its first open case from n = 15 to n = 57; and new **Conjecture D′** — the MOS lower and upper bounds are never both strict — with zero exceptions in 329 computed group values, its Sylow-split variant refuted by the predicted separating case C₃⊕C₈₇ |
