...
| [**Plus-minus Davenport constants**](conjectures/plus-minus-davenport/) | active | 1 | **CERTIFIED** — the last open plus-minus weighted Davenport constant at order ≤ 100 decided: **`D±(C₅⊕C₁₅) = 6`** (boxed {6,7} since Marchan–Ordaz–Schmid 2014, (secondary)), plus the smallest open family cases `D±(C₇⊕C₂₁) = 8` and `D±(C₃⊕C₄₅) = 7` — the first failing case of their C₃-family theorem is a genuine deficit — each value carried by three cross-checking engines with every witness re-verified from the definition; a from-scratch census of all **184** abelian groups of order ≤ 100 (exactly five miss the counting bound); **PROVED** attainment theorem T1 for `C₃⊕C₃ₙ` (presumptive MOS-regime rediscovery, marked) plus machine decisions moving that family's first open case from n = 15 toward n = 57; and new **Conjecture D′**: the MOS lower and upper bounds are never both strict — zero exceptions across every computed group |
