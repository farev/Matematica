...
| [**Plus-minus Davenport constants**](conjectures/plus-minus-davenport/) | active | 1 | **CERTIFIED** — the last open plus-minus weighted Davenport constant at order ≤ 100 decided: **`D±(C₅⊕C₁₅) = 6`** (boxed {6,7} since Marchan–Ordaz–Schmid 2014, (secondary)), plus three more smallest-open family cases — `D±(C₇⊕C₂₁) = 8`, `D±(C₃⊕C₄₅) = 7`, and `D±(C₅⊕C₅₅) = 8` (a 3.49-billion-node exhaustion showing C₅⊕C₁₅ is not the C₅-family's only deficit) — each value carried by cross-checking engines with every witness re-verified from the definition; a from-scratch census of all **184** abelian groups of order ≤ 100, extended to 127 (exactly five deficits, all ≤ 100); **PROVED** attainment theorem T1 for `C₃⊕C₃ₙ` (presumptive MOS-regime rediscovery, marked), settling that family for all n ≤ 56 bar one and moving its first open case from n = 15 toward n = 57; and new **Conjecture D′**: the MOS lower and upper bounds are never both strict — zero exceptions in 329 computed group values |
