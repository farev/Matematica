...
| [**Peaceable queens**](conjectures/peaceable-queens/) · [page ↗](https://fabianarevalo.com/peaceable-queens) | active | 3 | **CERTIFIED** — **a(16) = 37**, **a(17) = 42** and **a(18) = 47**: the three smallest open cases of OEIS A250000 decided (brackets [37, 64], [42, 72], [47, 81] since 2014) via exhaustive refutations of 38+38 (5.03×10⁹ nodes, two independent engines), 43+43 (2.15×10¹⁰ nodes, single engine) and 48+48 (NODES_SCI nodes, ≈ ENGINE_H core-hours, single engine — caveats in the README) with checker-verified witnesses, from a **PROVED** line-labeling reformulation with proved pruning lemmas; plus the whole ladder a(1..15) re-derived from scratch — the first reproducible derivation artifacts for a(14) = 28 and a(15) = 32 |
...
| [**Parking-polytope lattice points (A333331)**](conjectures/parking-polytope/) · [page ↗](https://fabianarevalo.com/parking-polytope) | active | 1 | **PROVED** — the lattice points of the parking-function polytope are counted by loop-graphs with all components unicyclic, with e.g.f. exp(−½log(1−T)+T/2−T²/4): both standing OEIS A333331 conjectures (Howroyd, Wiseman) proved at once, Selig's open enumeration question for stochastic-sandpile recurrent states (EJC 2024 §6) answered including asymptotics a(n) ~ 0.7465·n^{n−1/4}, plus **PROVED** the Ehrhart polynomial (answering Amanbayeva–Wang §6b; sum-form credits Liu–Thawinrak Dec 2025) and **CERTIFIED** a(1..40), the first terms past a(8) ever computed independently |
| [**Chromatic number of PG(7,2)**](conjectures/projective-chromatic/) · [page ↗](https://fabianarevalo.com/projective-chromatic) | active | 1 | **PROVED + CERTIFIED** — first computational attack on χ₂(8) ∈ {5,6} (Bishnoi–Cames van Batenburg–Ravi Problem 1; 5 would give R(3;5) ≥ 257): **Theorem — every proper 5-coloring of PG(7,2), if any exists, has a 2-group automorphism group** (Mersenne orders 3/7/31/127 killed by a proved orbit obstruction valid for all n, k; orders 17 and 5 by certified UNSATs, the order-5 legs machine-verified — [C,I] by the repo's own RUP checker, [C,C] by drat-trim; sole proviso: a two-line color-WLOG lemma, NOTE §4) — while an order-5-invariant witness **exists at n = 7**, and 1,000/1,000 sampled hyperplane restrictions fail to extend (50 DRUP-certified) |
...
| [**NCI da-trees (Amarilli–Monet–Suciu; refuted Wilhelm 2026)**](conjectures/nci-datrees/) · [page ↗](https://fabianarevalo.com/nci-datrees) | active | 1 | **CERTIFIED** — the first lower bound on the day-old open problem "smallest lattice with no winning dot-algebra tree": all **171,432,955 lattices with ≤ 15 elements** are winnable, in fact *left-linear* winnable (34.0×10⁹ posets streamed, counts matching OEIS A000112/A006966 at every size, dual implementations, 24/24 worst-case witnesses machine-verified), so the minimal counterexample — general (arXiv:2608.27416 §9) or left-linear (arXiv:2608.19414 §9) — has **≥ 16 elements** vs the ≈1.00011×10¹⁵ upper bound, no small lattice separates left-linear from general trees, and any counterexample set family needs > 15 distinct intersections |
| [**Kobon triangles (BBL 93–94 entry)**](conjectures/kobon-triangles/) · [page ↗](https://fabianarevalo.com/kobon-triangles) | active | 1 | **Rediscovery, marked as such** — `a^s_3(18) = 93`: no simple arrangement of 18 pseudolines has 94 triangles, decided by 561 dihedral-orbit SAT cubes with `drat-trim`-checked proofs (CERTIFIED), then found to be Blanc's 2008 theorem; plus a **flagged audit**: OEIS A006066's even-`n` upper bounds (94 at 18, 54 at 14) are general-position theorems applied to a problem whose records use triple points — on the cited literature `54 ≤ K(14) ≤ 55`, `93 ≤ K(18) ≤ 95` |
| [**Consecutive 8th-power residues (Erdős #436)**](conjectures/power-residue-pairs/) · [page ↗](https://fabianarevalo.com/power-residue-pairs) | active | 1 | **CERTIFIED** — the first explicit upper bound **`Λ(8,2) ≤ 1,508,324`** (Brillhart–Lehmer–Lehmer 1964 had only `≥ 1,200,744`): an exhaustive unit-orbit case tree over the 8th-power characters of small primes with `R(2)` even, 3,499,913 leaves all settled, independently checked; controls reproduce `Λ(2..7,2)` exactly and are sharp one below each |
| [**Graham rearrangement (Erdős #475)**](conjectures/graham-rearrangement/) · [page ↗](https://fabianarevalo.com/graham-rearrangement) | active | 1 | **CERTIFIED** — Graham's rearrangement conjecture closed for **every prime p ≤ 37**: all 70,066,181,009 subsets of F_p∖{0} verified via 1,954,471,973 dilation orbits (witness per orbit, Burnside-exact counts on all 173 cells; prior record was an uncertified 2016 check through order 25), moving the smallest undecided prime to **41** — where the dense half t ≥ 28 is also closed (228M orbits) — plus the zero-sum size-(p−3) layer, provably outside the HOS19 construction, **CERTIFIED** for all p ≤ 61 |
| [**Bit Deletion game (OEIS A398916)**](conjectures/bit-deletion/) · [page ↗](https://fabianarevalo.com/bit-deletion) | closed | 1 | **PROVED** — the complete Sprague–Grundy function of the Bit Deletion game: `G(n) = (L mod 2) + 2·[t odd]`, L the bit-length and t the number of initial odd-length zero-blocks of the binary expansion, settling both conjectures of the OEIS entry (values ≤ 3, `G(4n) = G(n)`) and giving the misère solution (P-positions = normal value 1) and exact counts (`2^{L−3}` high values per bit-length); the win/lose rule alone was Project Euler 961 folklore (marked as rediscovery); **CERTIFIED** from the definition for all n < 2^32 |
| [**Triangulation discrepancy, residue class 5 mod 6 (Basti–Cremaschi 2026)**](conjectures/triangulation-discrepancy/) · [page ↗](https://fabianarevalo.com/triangulation-discrepancy) | active | 1 | **PROVED + CERTIFIED** — first attack on the open residue class of the refined polychromatic-discrepancy bound disc(T) ≤ n − 2⌈(n+2)/3⌉ (arXiv:2608.21585, open for n ≡ 5 mod 6, checked by its authors at n = 11 only): the bound **PROVED for n = 11, 17, 23** (the first three open orders) and for every n in the class whenever the balanced 4-colouring's big class has at most two vertices of degree ≥ 5, via a structure theorem for any counterexample (which must have n ≥ 29, a 4-colouring with classes ((n−1)/2, ·, ·, ·), a fully mixed big class with ≥ 2m+3 degree-3 vertices and 3..m−1 of degree ≥ 5, every other vertex on a face avoiding it) and an Euler-formula count; **CERTIFIED** at the fourth open order n = 29 (≈ 4·10¹² triangulations, reduced by the structure theorem to plantri's 10⁹ triangulations on 18 vertices; 60,070,002 candidates, all of discrepancy 1) and as independent confirmations at n = 17 (full census of 129,664,753 triangulations, 2,652 extremal) and n = 23 (948,057 structural candidates, all of discrepancy 1) — plus the exact discrepancy distribution for 13 ≤ n ≤ 17; a counterexample needs n ≥ 35 |
| [**Antidiagonal traffic anomaly (Gil–Liang–Odetola–Weiner 2026)**](conjectures/antidiagonal-anomaly/) · [page ↗](https://fabianarevalo.com/antidiagonal-anomaly) | closed | 1 | **PROVED** — Conjecture 7.4 of arXiv:2609.01562 (three days old, verified by its authors to n = 495): an obstruction on the antidiagonal of the n×n path grid never moves the point of maximal traffic to the boundary for any n ≥ 496 (Robbins–Stirling bound ρ(n) < 0.9939 for n ≥ 3000, rational-arithmetic certified; exact integer check for 496 ≤ n ≤ 2999, worst case ρ(497) = 0.999955), the criterion reduced to the integer inequality a(n−2a+1)·C(n,a)² > (n−1)·C(2n−2,n−1) (obstruction deficit exceeds the Catalan number Cat(n−1)), the limit ρ(n) = √(8/π)e^{−1/2}(1 + 1/√(2n) + O(1/n)) = 0.9679…, and the paper's "irregular" sporadic pattern (82 values, listed) explained as parity-alternating rounding of the real maximiser |
