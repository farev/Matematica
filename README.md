...
| [**Chromatic Ramsey numbers F(j,k) (Sawin, MO 513849)**](conjectures/chromatic-ramsey/) | active | 1 | **PROVED** — Sawin's F(3,k), the largest n with K_n split into k triangle-free 3-colourable graphs, satisfies F(3,k) ≥ max_t C(k,t)·2^{k−t} ≥ 3^k/(k+1), so its growth rate is exactly 3 (previous rate 2^{3/2}), and F(j,k) ≥ j^k/poly(k) for every fixed j (unconditional for j ≤ 4); plus the first exact values, **CERTIFIED**: F(3,3) = 14 (DRUP-checked), F(4,3) = 16, PENDING_INDEX_F34 F(3,5) ≥ 122, F(4,4) ≥ 44, with Wiesner's Stirling-sum conjecture exact at (3,3) but not at (4,3), and its j = 3 value (3^k+1)/2 realised for k ≤ 5 by the even-weight ternary words |
