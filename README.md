...
| [**Plus–minus Davenport constants**](conjectures/plus-minus-davenport/) · [page ↗](https://fabianarevalo.com/plus-minus-davenport) | active | 1 | **CERTIFIED** — `D±(C₅⊕C₁₅) = 6`: the last unknown plus–minus weighted Davenport constant among all groups of order ≤ 100 (bracketed {6,7} since 2014) decided at the *lower* end, by four independent verification paths and a maximality certificate over all 85,155 extremal sets; `D±(C₇⊕C₂₁) = 8` (first determination, *upper* end of its bracket), with the extremal sequence **unique up to Aut(G) and signs** (one orbit of 2016 = \|GL(2,7)\| sets); plus the complete certified `D±` table for every abelian group of order ≤ 150, matching every published value in range |
