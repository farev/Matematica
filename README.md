...
| [**Plus–minus Davenport constants**](conjectures/plusminus-davenport/) | active | 1 | **CERTIFIED** — **D±(C₅⊕C₁₅) = 6**: the one cell of the Marchan–Ordaz–Schmid table (all \|G\| ≤ 100) left open since 2014, decided by four independent implementations (two agreeing on exact node counts; census of all 85 155 extremal sets), with 3 of 5 nonexistence strata upgraded to **PROVED**; plus an independent recomputation of the entire ≤ 100 table (184 groups; 12 of 17 bracket-open cells attain the pigeonhole bound — C₅⊕C₁₅'s lower-bound behavior is the exception) and apparently-first values past order 100 (complete through 135, targeted to 243: D±(C₇⊕C₂₁) = D±(C₁₃⊕C₁₃) = D±(C₅⊕C₃₀) = 8, first strictly-between cell at C₃⊕C₃⊕C₁₅), all consistent with new machine-verified **Conjecture A**: d± is either ⌊log₂\|G\|⌋ or splits over a direct sum |
