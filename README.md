...
| [**Circular repetition thresholds**](conjectures/circular-thresholds/) | active | 1 | **PROVED** a pumping lemma making `CRT_W(n) = RT(n)` — open for `4 ≤ n ≤ 44` — decidable by two finite searches, with the machinery re-deriving the known `n = 3` case end to end, plus **PROVED** that the shift-equivariant morphisms that work at `n = 3` provably cannot work at `n ≥ 4`, and **CERTIFIED** spectra of realizable lengths for `n = 3,4,5,6` (the `n = 4` spectrum has late gaps at 147 and 154). No open case settled |
