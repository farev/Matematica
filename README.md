...
| [**Good permutations and Mersenne primes (MO 514690)**](conjectures/good-permutations/) | active | 1 | **CERTIFIED + PROVED** — Weiss's question (Aug 2026: for odd n, does a permutation of [n] with no consecutive block of integer average exist iff n is a Mersenne prime?) settled at its first undecided case: **no good permutation of {1..63} exists** (2^57-candidate structured search, two independent engines with identical node counts 1,433,402,570, 344 s / 128 s), so the answer is "iff Mersenne prime" for all odd n ≤ 127; plus **PROVED** that Weiss's family 1, p−1, p, p−3, p−2, … is good exactly when p is prime (its odd prefix of length L sums to −p mod L, nothing else ever fails), the triangular structure theorem with the even-length lemma (only odd blocks matter), and exact counts for all n ≤ 31 (W(p) unique up to symmetry at 7 and 31) |
