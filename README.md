...
| [**Plus–minus Davenport constants**](conjectures/plus-minus-davenport/) · [page ↗](https://fabianarevalo.com/plus-minus-davenport) | active | 1 | **CERTIFIED** — `D±(C₅⊕C₁₅) = 6`: the last unknown plus–minus weighted Davenport constant among all groups of order ≤ 100 (bracketed {6,7} since 2014) decided at the *lower* end, by four independent verification paths and a maximality certificate over all 85,155 extremal sets; `D±(C₇⊕C₂₁) = 8` (first determination, *upper* end), with the extremal sequence **unique up to Aut(G) and signs** (one orbit of 2016 = \|GL(2,7)\| sets); the complete `D±` table for all 312 abelian groups of order ≤ 162, matching every published value in range, with 15 new gap cells — two of them (`C₃²⊕C₁₅` at 135, `C₃³⊕C₆` at 162, both = 7 in brackets {6,8}) the first values shown to lie **strictly between** the Marchan–Ordaz–Schmid bounds |
