...
| [**Circular repetition thresholds**](conjectures/circular-thresholds/) · [page ↗](https://fabianarevalo.com/circular-thresholds) | active | 1 | **PROVED** a pumping lemma making `CRT_W(n) = RT(n)` — open for `4 ≤ n ≤ 44` — decidable by two finite searches, verified end to end by re-deriving the known `n = 3` case; and **PROVED** (computer-assisted) that the shift-equivariant normal form which works at `n = 3` admits no threshold fixed point at any morphism length for `4 ≤ n ≤ 9`; plus **CERTIFIED** spectra of realizable lengths for `n = 3,4,5,6`, the `n = 4` one having late gaps at 147 and 154 that break an apparently settled run. No open case settled |
