...
| [**Ordinary lines of 15 points (Sylvester's problem, A003034)**](conjectures/ordinary-lines/) | active | 1 | **CERTIFIED (partial)** — first computational attack on the smallest open case of the Dirac–Motzkin conjecture, t₂(15) ∈ {7, 8, 9}: a 15-point set (real or pseudoline) with only 7 ordinary lines must have exactly two 5-point lines (PROVED via Melchior + counting), and those lines cannot be disjoint — 261 SAT sub-cases, all UNSAT with drat-trim-checked proofs — nor meet with no ordinary line joining them (411 arrays, all UNSAT); the remaining meeting case (45 sub-cases, ≈ 151 000 arrays, ≈ 25 CPU-h measured) is **open**, so t₂(15) ≥ 8 is not yet established; plus t₂(20) = 10, t₂(24) = 12 read off the literature |
| [**Good permutations and Mersenne numbers (MO 514690)**](conjectures/good-permutations/) | active | 1 | **CERTIFIED + PROVED** — the first undecided case of the "good permutations exist iff n is a Mersenne prime" question is settled: **no good permutation of {1..63}** (2^57 candidates after a PROVED 2-adic reduction to binary-tree automorphisms, exhausted by two independent programs, 1,433,402,570 nodes each, 20 s); the asker's construction is good **iff** p is prime (PROVED); lemma-free brute force to n = 43 with exact counts 2, 4, 4 at n = 3, 7, 31; so the conjecture holds for all odd n < 255 |
