...
`ATTEMPTED.md` is the machine-facing index: one line per problem, generated by
`tools/build_index.py`, never edited by hand. **Start a session by reading it
instead of sweeping `conjectures/` and `log/`.** It costs about 1k tokens where
that sweep costs about 166k, and, unlike anything derived from main alone, it
lists the problems that have been attacked only on unmerged branches. That
blind spot is not hypothetical: six independent sessions attacked the same
plus/minus Davenport problem under three directory names because none of them
could see the others' branches. A research session reads it with `--stdout`, which prints without touching the
working tree; regenerating in place on a branch would make every branch carry a
different header and every merge conflict on it. Refresh the committed copy on
main after a merge.

```bash
git fetch --all --prune && python3 tools/build_index.py --stdout   # to read
python3 tools/build_index.py                                       # to refresh
```
